let users = [
  ...
  const cors0ptions = { origin: '*'}; //CONFIGURATION DU CORS

  const app = express(); //INITIALISATION DU MODULE EXPRESS

  app.use(cors(cors0ptions)); //UTILISATION DU MODULE CORS PAR EXPRESS

  app.listen(3000); //MISE EN ECOUTE DU SERVEUR

  //On traite les requêtes de type GET sur la route http://localhost/3000:players
  ...
  app.get('/users/:id', function(req,res){
    ...
    let id = req.params.id; //On récupère l'ID présent en bout de root
